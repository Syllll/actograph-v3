...
function prepare(obj) {
  return obj !== undefined && obj !== null && typeof obj.export === 'function'
    ? obj.export()
    : obj;
}
 ...
 * @param {*} obj
 */
class QInt8 extends QClass {
  /**
   * @function read
   * @memberof module:qtdatastream/types.QInt8
   * @static
   * @param {Buffer} buffer
   * @returns {number} Buffer coerced to boolean
   */
  static read(buffer) {
...
class QDateTime extends QClass {
   ...
   */
  static read(buffer) {
    const julianDay = QInt64.read(buffer);
    const msecondsSinceMidnight = QUInt.read(buffer);
    const _isUTC = QInt8.read(buffer);
    console.log(_isUTC, julianDay, msecondsSinceMidnight);
  ...
  toBuffer() {
    const bufs = [];
    const milliseconds =
      (this.__obj.getTime() - this.__obj.getTimezoneOffset() * 60000) %
      86400000;
    const julianday = dateToJulianDay(this.__obj);
    bufs.push(QInt64.from(julianday).toBuffer());
    bufs.push(QInt64.from(milliseconds).toBuffer());
    bufs.push(QInt8.from(this.__obj.getTimezoneOffset() === 0).toBuffer());
    return Buffer.concat(bufs);
  }
...
class QUserType extends QClass {
    ...
    return class extends QUserType {
      ...
      toBuffer(skipname = false) {
        const bufs = [
          ...
          qclass.from(this.__obj).toBuffer(true),
        ];
        return Buffer.concat(bufs);
      }
    };
  ...
  toBuffer(skipname = false) {
    const bufs = [
      this._getNameBuffer(skipname),
      QUserType.usertypes.get(this.name).from(this.__obj).toBuffer(true),
    ];
    return Buffer.concat(bufs);
  }
