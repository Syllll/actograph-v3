...
export const useBuilderjson = (options: {
  ...
  const methods = {
    postUpdateMessage: () => {
      options.postMessage(ESyncMessageType.updatePageBuilderJson, <
        IContentUpdatePageBuilderJson
      >{ pageBuilderJson: options.tree.sharedState.pageBuilderJson });
    },
    handleMessage(content: any) {
      ...
      if (
        !builderJsonReceived ||
        JSON.stringify(builderJsonReceived) ===
          JSON.stringify(options.tree.sharedState)
      ) {
        return;
      }
  ...
  watch(
    ...
    () => {
      options.postMessage(ESyncMessageType.drop, <Partial<IDragSharedState>>{
