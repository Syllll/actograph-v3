<template>
  <div>
    <slot name="header" v-bind="{ stepInfos: builder.stepInfos?.value, builder }" />

    <!-- ? CURRENT STEP STUFF -->
    <div v-if="builder.sharedState.step" :class="`${props.styles?.steps.container}`">
      <!-- ? NAME -->
      <h3 v-if="builder.sharedState.step?.display" :class="`${props.styles?.steps.title}`">
        <slot :name="builder.sharedState.step.value + '-title'" v-bind="{ step: builder.sharedState.step }">
          {{ builder.sharedState.step?.name }}
        </slot>
      </h3>

      <!-- ? DESC -->
      <span v-if="builder.sharedState.step?.display && builder.sharedState.step?.desc" :class="`${props.styles?.steps.desc}`">
        <slot :name="builder.sharedState.step.value + '-desc'" v-bind="{ step: builder.sharedState.step }">
    ...
    <div :class="classMapper.methods.mapRowClasses(props.styles?.row)">
      ...
      >
        <component
          ...
          :class="classMapper.methods.mapColClasses(props.styles?.field, { errored: field.error })"
      ...
      <slot name="footer" v-bind="{ stepInfos: builder.stepInfos?.value, builder }" />

      <slot name="actions" v-bind="{ stepInfos: builder.stepInfos?.value, builder }">
        <div class="col-12 q-pa-md row justify-center q-gutter-sm">
          ...
          <slot name="previous" v-bind="{ stepInfos: builder.stepInfos?.value, builder }" >
            <q-btn
              v-if="!builder.stepInfos?.value?.isFirstStep && builder.stepInfos?.value?.hasStep"
              flat
          ...
          <slot name="submit" v-bind="{ stepInfos: builder.stepInfos?.value, builder }" >
            <q-btn
              flat
              :class="`
                smooth
                bg-primary${builder.sharedState.allowSubmit ? ''  : '-10 not-clickable'}
              `"
              :icon="!builder.stepInfos?.value?.hasStep || builder.stepInfos?.value?.isLastStep ? 'mdi-check' : 'mdi-chevron-right'"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const builder = useBuilder({ model: props.modelValue, fields: props.fields as IField[], emit }) as any
