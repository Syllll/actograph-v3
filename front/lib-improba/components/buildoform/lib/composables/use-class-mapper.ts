...
import { IFieldStyle, IRowStyle, IColStyle } from '../interfaces/style.interface'
...
export const useClassMapper = () => {
  ...
  const methods = {
    mapRowClasses (style: IRowStyle): string {
      ...
      classes.push(direct.map((attr: any) => (style)[attr as keyof IRowStyle] ? attr : ''))
      classes.push(onlyVal.map((attr: any) => (style)[attr as keyof IRowStyle] ? (style)[attr as keyof IRowStyle] : ''))
      classes.push(keyVal.map((attr: any) => (style)[attr as keyof IRowStyle] ? `${attr}-${(style)[attr as keyof IRowStyle]}` : ''))
      classes.push(QKeyVal.map((attr: any) => (style)[attr as keyof IRowStyle] ? `q-${attr}-${(style)[attr as keyof IRowStyle]}` : ''))
      // console.log({ classes })
    ...
    mapColClasses (style: IFieldStyle, options: { errored: boolean }): string {
      ...
      const mapped = Object.entries(style)
        .map(([name, value]) => {
          ...
          if (['static', 'directives'].includes(name)) { return '' }
