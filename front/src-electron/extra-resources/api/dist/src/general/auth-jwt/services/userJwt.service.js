...
let UserJwtService = class UserJwtService {
  ...
  async create(userCreateDto, emitUserCreateSignal = true) {
    ...
    if (emitUserCreateSignal) {
      await this.eventEmitter.emitAsync(
        'authJwt.userCreated',
        new authJwtUserCreated_event_1.authJwtUserCreatedEvent(savedUser)
      );
    }
    return {
      id: savedUser.id,
  ...
  async activate(activationToken) {
    ...
    try {
      ...
      await this.eventEmitter.emitAsync(
        ...
        new authJwtUserActivated_event_1.authJwtUserActivatedEvent(user)
      );
      return result;
    } catch (error) {
  ...
  async sendMailForNewPassword(email, resetPasswordUrl) {
    ...
    await this.eventEmitter.emitAsync(
      ...
      new authJwtPasswordForgot_event_1.authJwtPasswordForgotEvent(user)
    );
