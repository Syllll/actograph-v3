...
let AuthJwtController = class AuthJwtController extends base_controller_1.BaseController {
    ...
    async postLogin(username, password) {
        ...
        if (!user)
            throw new common_1.HttpException('Wrong credentials', 403);
    ...
    async postRegister(user) {
        if (this.allowRegisterNewUserFromAuthJwt) {
            ...
            throw new common_1.HttpException('Creation  not authorized', common_1.HttpStatus.UNAUTHORIZED);
        }
    }
    async getNewPassword(username, resetPasswordUrl) {
        const response = new userJwt_service_1.SuccessResponse();
        const result = await this.usersService.sendMailForNewPassword(username, resetPasswordUrl);
        if (!result)
            throw new common_1.HttpException('Récupération impossible', common_1.HttpStatus.BAD_REQUEST);
        response.success = true;
        response.message = JSON.stringify('Mail de récupération de mot de passe envoyé');
        response.errors = [];
        return response;
    }
    async postNewPassword(token, password) {
        ...
        if (!result.user)
            throw new common_1.HttpException('Impossible to change password', common_1.HttpStatus.BAD_REQUEST);
...
], AuthJwtController.prototype, "postLogin", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], AuthJwtController.prototype, "refreshToekn", null);
__decorate([
...
], AuthJwtController.prototype, "logout", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], AuthJwtController.prototype, "postRegister", null);
__decorate([
    ...
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], AuthJwtController.prototype, "getNewPassword", null);
__decorate([
    ...
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], AuthJwtController.prototype, "postNewPassword", null);
