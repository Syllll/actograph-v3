...
let SecurityService = class SecurityService {
    ...
    getLocalUsername() {
        ...
        if (mode !== 'electron') {
            throw new common_1.InternalServerErrorException('Local user is only available in electron mode');
        }
    ...
    async activateLicense(key) {
        ...
        const response = await axios_1.default.post(`${process.env.ACTOGRAPH_API}/license`, {
            key: key,
            password: process.env.ACTOGRAPH_API_PASSWORD
        });
        const responseData = response.data;
        if (responseData.message) {
            throw new common_1.BadRequestException((_a = responseData.message) !== null && _a !== void 0 ? _a : 'Unknown error when checking licence, code: sec.ser.46');
        }
        return true;
    }
