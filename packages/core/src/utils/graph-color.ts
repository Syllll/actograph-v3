/**
 * Mots-clés CSS/SVG (insensible à la casse) → hex #rrggbb.
 * Couvre les noms de couleur Qt enregistrés dans les anciens fichiers .chronic.
 */
...
/**
 * Normalise une couleur protocole (hex, rgb/rgba ou nom CSS/Qt) vers un format
 * accepté par amCharts5 (#rrggbb ou rgb/rgba inchangé).
 */
export function normalizeGraphColor(
  color: string,
  fallback: string,
): string {
  ...
  if (trimmed.startsWith('#')) {
    ...
    if (HEX6_PATTERN.test(hexBody)) {
      return `#${hexBody.toLowerCase()}`;
    }
    if (HEX8_PATTERN.test(hexBody)) {
      return `#${hexBody.slice(0, 6).toLowerCase()}`;
    }
    if (HEX3_PATTERN.test(hexBody)) {
      return expandShortHex(hexBody);
    }
    return fallback;
  }
